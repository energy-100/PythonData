...
def printPath(pathtemp):
    ...
    for fl in fileList:
        ...
        dx = np.linspace(0.1,0.1,len(x)).tolist()
        dy = np.linspace(0.1,0.1,len(x)).tolist()
        dz = np.linspace(0.5,0.5, len(x)).tolist()
        #dz = hist.flatten()
        ax.bar3d(x,z,dz,0.2,0.001,y, color=color, alpha=1,label=fl)
