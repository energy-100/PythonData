...
def printPath(pathtemp,fun,num):
    ...
    for fl in fileList:
        ...
        if(fun==4 or fun==2):
            ...
            plt.scatter(x, y, color=color, label=fl.split(".")[0].split("-")[1].rstrip(string.digits), marker=marklist[markernum], s=20)  # 佳蕾姐数据
        elif(fun==6):
    ...
    for fl in fileList2:
        ...
        if(fun==1):
            ...
            flmain =fl.split(".")[0].split("-")[1].rstrip(string.digits)    #佳蕾姐拟合
    ...
    plt.text(2.5, 25000, "${I}$ = ${I_0}$*(1+${t_0}$/${\\tau}$)${^{-\gamma}}$+D", size=28,style="italic")
...
# printPath("C:/Users/ENERGY/Desktop/工作文件/张老师穴位前/",4,0)
