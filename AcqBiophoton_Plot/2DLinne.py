...
def printPath(pathtemp,fun,num,inverted):
    ...
    col_labels=[]
    row_labels=[]
    colorlist=['blue','red','black','yellow','green']
    ...
    if (fun == 1):
        fitingpath=pathtemp+"拟合/"
    if (fun == 2):
        fitingpath=pathtemp+"指数拟合/"
        paras.append(["测量点", "${I_0}$", "${\\tau}$", "D", "${R^2}$"])
    if (fun == 3):
        fitingpath=pathtemp+"拟合/"
    if (fun == 4):
        fitingpath=pathtemp+"双曲线拟合/"
        paras.append(["测量点", "${I_0}$", "${\\tau}$", "${\gamma}$", "D", "${R^2}$"])
    if (fun == 5):
        fitingpath = pathtemp + "指数积分形式拟合/"
        paras.append(["测量点", "${I_0}$", "${\\tau}$", "D", "${R^2}$"])
    if (fun == 6):
        fitingpath=pathtemp+"双曲线积分形式拟合/"
        paras.append(["测量点","${I_0}$","${\\tau}$","${\gamma}$","D", "${R^2}$"])
    ...
    colornum=0
    if(inverted==1):
        for i in range(math.floor(len(fileList2)/2)):
            temp= fileList2[i];
            fileList2[i]=fileList2[len(fileList2)-1-i];
            fileList2[len(fileList2) -1- i]=temp;
    ...
    for fl in fileList2:
        ...
        # colornum += 1
        if (fun == 4 or fun == 2):
            plt.scatter(x, y, color=color, label=re.sub(r'[A-Za-z]', "", fl.split("-")[1].split("（")[0]), marker="*")  # 张老师数据
            # plt.scatter(x, y, color=color, label=fl.split(".")[0], marker="*", s=0.8)  # 佳蕾姐数据
        elif (fun == 6 or fun == 5):
            xbar = np.asarray(x) + (dt / 2)
            ybar = np.asarray(y) / dt
            plt.bar(xbar, ybar, color=color, width=dt, alpha=0.3,label=re.sub(r'[A-Za-z]', "", fl.split("-")[1]).split("（")[0]+"实验数据")
            # plt.scatter(x,y,color=color,label=fl.split(".")[0],marker="*",s=0.8)        #佳蕾姐数据
            # plt.scatter(x,y,color=color,label=re.sub(r'[A-Za-z]',"", fl.split("-")[1]),marker="o")
        ylist.append(y)
        ...
        if(fun==1):
            ...
            paras.append([flmain,s1,s3,s4,s5,r2])
            flmain2 = flmain + "双曲线拟合"
        elif(fun==5):
            print(eachdata)
            s1 = float(eachdata[0])
            s2 = float(eachdata[1])
            ...
            flmain = re.sub(r'[A-Za-z]', "", fl.split("-")[1])
            paras.append([flmain,s1,s2,s3,r2])
            flmain2 = flmain + "指数积分拟合"+'指数积分拟合(${I}$ =$\int_t^{t+ \Delta t}$(${I_0}$e${^{-t/\\tau}}$+D)dt)'
            # flmain=fl  #姐蕾姐数据
            ...
            flmain = re.sub(r'[A-Za-z]', "", fl.split("-")[1]).split("（")[0] #张老师数据必要过程
            # flmain = flmain + "双曲线积分拟合" + "(优度：" + eachdata[-3] + ")"
            print(flmain)
            paras.append([flmain,s1,s3,s4,s5,rs])
            flmain2 = flmain + "双曲线积分拟合"+"(${I}$ =$\int_t^{t+ \Delta t}$(${I_0}$*(1+${t}$/${\\tau}$)${^{-\gamma}}$+D)dt)"
    ...
    plt.figure()
    plt.subplot(1, 1, 1,frameon=True, xticks=[], yticks=[])
    ...
    # the_table=plt.table(cellText=paras, colWidths=[0.1] * len(paras),
    #            fontsize=5, loc='center',cellLoc='center')
    the_table=plt.table(cellText=paras,fontsize=40,
...
# printPath("C:/Users/ENERGY/Desktop/工作文件/finall2/",6,1)
# printPath("C:/Users/ENERGY/Desktop/工作文件/finall2/",6,2)
# printPath("C:/Users/ENERGY/Desktop/工作文件/finall2/",6,3)
# printPath("C:/Users/ENERGY/Desktop/工作文件/finall2/",6,4)
# printPath("C:/Users/ENERGY/Desktop/工作文件/finall2/",6,5)
...
printPath("C:/Users/ENERGY/Desktop/工作文件/finall2/",6,0,1)

# printPath("C:/Users/ENERGY/Desktop/工作文件/lhy/",5,0)
